        ...
        *args, **kwargs
        ...
        if validate:
            ...
            validation_loader = self._create_data_loader(
                ...
                num_workers=num_workers,
                shuffle=False)
            filter_filepath = os.path.join(data_dir, dataset, filter_file_val) if filter_file_val else None
            filter_map = get_filter_map(filter_filepath)
        self._fit(
...
class XModelIS(ModelIS):
    ...
    * Implicit sampling: negatives are not explicity sampled but
    ...
    def _predict(self, dataset, top_k, batch_size, num_workers, beta, **kwargs):
        """
    ...
    def rerank(self,
        ...
        filter_map = np.loadtxt(os.path.join(data_dir, dataset, filter_map)).astype(np.int) if filter_map else None
        ...
        pred_labels = scipy.sparse.load_npz(rerank_file)
        ...
        classifiers = self.net.get_clf_weights()
        classifiers = classifiers/np.linalg.norm(classifiers, axis=1, keepdims=True)

        if not os.path.exists(rerank_file):
            raise Exception(rerank_file)

        reranked_labels = scipy.sparse.csr_matrix(pred_labels.shape)

        num_beams = pred_labels[0].indices.shape[0]
        num_docs = pred_labels.shape[0]

        pred_labels.sort_indices()
        rows, cols = pred_labels.nonzero()
        indices = cols.reshape(num_docs, num_beams)

        lbl_weight = classifiers[cols].reshape(num_docs, num_beams, -1)
        lbl_weight = lbl_weight.transpose((0, 2, 1))

        scores = doc_embeddings[:, None, :]@lbl_weight
        scores = scores.squeeze()

        reranked_labels[np.arange(num_docs)[:,None], indices] = scores

        basename = os.path.basename(rerank_file).split(".")[0]
        save_file = f"{os.path.dirname(rerank_file)}/{basename}-rerank"
        scipy.sparse.save_npz(save_file, reranked_labels)
    ...
    def predict(self,
        ...
        * Also prints prediction time, precision and ndcg
        ...
        predicted_labels = self._predict(
            dataset, top_k, batch_size, num_workers, beta, **kwargs)
        time_end = time.time()
