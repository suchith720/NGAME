    ...
    * Assumes diagonal entries are ground truth
    ...
    def forward(self, output, target, *args):
        ...
        if target.size(0) != target.size(1):
            ...
            k = self.num_positives if sim_p.shape[1] > self.num_positives else sim_p.shape[1]
            indices = sim_p.topk(largest=False, dim=1, k=k)[1]
        ...
        k = self.num_negatives if sim_m.shape[1] > self.num_negatives else sim_m.shape[1]
        indices = sim_m.topk(largest=True, dim=1, k=k)[1]
